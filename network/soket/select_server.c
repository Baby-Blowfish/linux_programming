            ...
            if(FD_ISSET(client_fd[start_index],&readfd))
            {
                ...
                if((n = read(client_fd[start_index],mesg,sizeof(mesg))) >= 0)
                {
                    if( n == 0)
                    {
                        ...
                        printf("close csock fd %d\n",client_fd[start_index]);

                    }
                    mesg[n]='\0';
                    printf("Received data from soket(fd : %d) : %s",client_fd[start_index],mesg);
