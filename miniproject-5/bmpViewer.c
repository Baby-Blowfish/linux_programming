...
int main(int argc, char **argv)
{
    ...
    int depth_fb = vinfo.bits_per_pixel / 8.;  // 프레임버퍼의 픽셀당 바이트 수 : 2byte
    ...
    pBmpData = (u2byte *)malloc(vinfo.xres * vinfo.yres * depth_fb);
    if (pBmpData == NULL)
    {
    ...
    // 프레임버퍼 메모리 맵핑
    pFbMap = (u2byte *)mmap(0, vinfo.xres * vinfo.yres * depth_fb, PROT_READ | PROT_WRITE, MAP_SHARED, fbfd, 0);
    if (pFbMap == (u2byte *)-1)
    {
        perror("mmap()");
        free(pData);
    ...
    for (int y = 0, k, total_y; y < rows; y++)  // BMP 이미지 크기와 프레임버퍼 크기를 비교하여 제한
    {
        ...
        total_y = y * vinfo.xres;  // fb의 y축의 시작 위치 계산
        ...
        for (int x = 0; x < cols; x++)  // 가로 방향에서도 프레임버퍼 크기만큼 제한
        {
            ...
            *(pBmpData + x + total_y) = pixel;  // BMP 데이터를 메모리에 저장
        }
    }
