...
int main(int argc, char *argv[])
{
	...
	// 클라이언트의 이름을 서버에 알림
	char *buf = (char *)malloc(BUFSIZE * sizeof(char));  // BUFSIZE 크기만큼 메모리 할당
	if (buf == NULL) {
	...
	// 데이터 보내기(가변 길이)
	retval = send(client_info.sock, buf, len, 0);
	if (retval == SOCKET_ERROR) {
		err_display_msg("send()");
		return -1;
	}
