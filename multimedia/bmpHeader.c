...
int readBmp(char *filename, unsigned char **data, int *cols, int *rows)
{
    ...
    {
        // 에러 메시지 출력 후 파일 닫기
        perror("This image file doesn't support 24-bit color\n");
        fclose(fp);
        return -1;  // 실패 시 -1 반환
    }
    ...
    *rows = bmpInfoHeader.biHeight;  // 이미지의 세로 크기
    ...
    fseek(fp, bmpHeader.bfOffBits, SEEK_SET);
		...
		int imageSize = bmpInfoHeader.SizeImage;

		if (imageSize == 0) {
				imageSize = bmpInfoHeader.biWidth * bmpInfoHeader.biHeight * (bmpInfoHeader.biBitCount / 8);
		}
