...
#define CAMERA_COUNT 10          // 캡처할 카메라 프레임 수
...
int captureImage(int fd)
{
	...
	char filename[50];
	sprintf(filename, "captureimage%02d.bmp", i+1); // 'captureimage01.bmp', 'captureimage02.bmp' ... 형식으로 파일 이름 생성

	Mat saveImage = Mat(image.size(), CV_8UC3, buffer);
	imwrite(filename, saveImage);  // 생성한 파일 이름으로 이미지 저장
